...
SiteUtils._domainPattern = /^[\w\u0080-\uffff][\w\-\.\u0080-\uffff]*$/;

SiteUtils._ios = CC["@mozilla.org/network/io-service;1"]
    .getService(CI.nsIIOService);

SiteUtils._uriFixup = CC["@mozilla.org/docshell/urifixup;1"]
    .getService(CI.nsIURIFixup);

SiteUtils.sorter = function(a, b) {
  if (a == b)
    return 0;
  if (!a)
    return 1;
  ...
  return dp.test(a) ? (dp.test(b) ? (a < b ? -1 : 1) : -1) : (dp.test(b)
      ? 1
      : a < b ? -1 : 1);
};

SiteUtils.sort = function(ss) {
  return ss.sort(sorter);
};

SiteUtils.getSite = function(url) {
  if (!url || url.charCodeAt(0) < 33 && // needs trimming
  ...
  try {
    ...
    switch (scheme) {
      case "http" :
      case "https" : // commonest case first
        break;
      case "javascript" :
      case "data" :
        return "";
  ...
  try {
    ...
    try {
      ...
      return /^\/\/[^\/]/.test(host)
          && (host = this.domainMatch(host.replace(/^\/\/([^\/]+).*/, "$1")))
          ? scheme + "//" + host
          : scheme;
    }
  } catch (ex) {
...
SiteUtils.list2set = function(sl) {
  // kill duplicates
  var prevSite = "";
  var site;
...
SiteUtils.sortedSet = function(sl) {
  return this.list2set(this.sort(sl));
};

SiteUtils.splitString = function(s) {
  return s && /[^\s,]/.test(s) && s.split(/\s*[,\s]\s*/) || [];
};

SiteUtils.domainMatch = function(url) {
  const m = url.match(_domainPattern);
  return m ? m[0].toLowerCase() : "";
};

SiteUtils.sanitizeList = function(sl) {
  for (var j = sl.length; j-- > 0;) {
...
SiteUtils.sanitizeMap = function(sm) {
  var site;
  delete sm[""];
  for (var url in sm) {
...
SiteUtils.sanitizeString = function(s) {
  // s = s.replace(/,/g,' ').replace(/\s{2,}/g,'
  // ').replace(/(^\s+|\s+$)/g,'');
  return this.set2string(this.string2set(s));
};

SiteUtils.string2set = function(s) {
  return this.sortedSet(this.sanitizeList(this.splitString(s)));
};

SiteUtils.set2string = function(ss) {
  return ss.join(" ");
};

SiteUtils.crop = function(url, width, max) {
  width = width || 100;
  if (url.length < width)
    return url;
